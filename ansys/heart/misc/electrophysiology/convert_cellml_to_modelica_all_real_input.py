            ...
            if "var" in i: 
                    ...
                    if var[0]!= 'time':
                        ...
                        if 'init:' in i:
                            ...
                            var.append('No init')
                        if "pub:" in i:
                            var.append(i[i.index('pub:'):].split()[1].replace(',','').replace('};',''))
                        else:
                            var.append('No pub')
                        if 'priv:' in i:
                             var.append(i[i.index('priv:'):].split()[1].replace(',','').replace('};',''))
                        else:
                            var.append('No priv')
                        if comp_name =='cell_geometry':
                            print(i)
...
def convert_equation_cellml_to_modelica(eq,var_in_out,comp):
    ...
    while 'pow('in  eq:
        ...
        eq =eq.replace(eq[eq.index('pow('):end_index+1],'('+eq[eq.index('pow(')+4:exp_index]+')^('+eq[exp_index+1:end_index].replace(' ','')+')')
    ...
    while 'ode(' in  eq:
        ...
        while eq[end_index] != ')':
            end_index+=1 
        eq =eq.replace(eq[eq.index('ode('):end_index+1],'der('+eq[eq.index('ode(')+4:exp_index]+')')
    ...
    for j in var_in_out:
        if  eq not in comp[len(comp)-len(var_in_out):] and j in eq:
            ...
            for i in range(len(eq_split)-1):
                ...
                if eq_split[i+1].isalpha() or eq_split[i+1][0]=='_' or eq_split[i][-1].isalpha() or eq_split[i][-1]=='_':
    ...
    with open("Units.mo",'w') as f:
        ...
        for unit in units:
                ...
                if units[unit] != []:
                    ...
                    if s.count('/') >1:
                        s = '.'.join(s.rsplit('/', 1))
                    f.write('type '+ u+' = Real ( final quantity=\"'+u+'\" , final unit = \"'+s + '\");\n')
        f.write('end Units;')
...
with open('Components.mo','w') as f:
    ...
    for comp in comp_var:
        ...
        f.write(' model '+replace_lower_case_by_capital(comp)+' \"'+replace_lower_case_by_capital(comp)+'\"\n')
        compute_var=[]
        var_in_out = []
        ...
        for var in comp_var[comp]:
            ...
            if var[3] =='in' or var[4]=='in':
                if var[4]!='out' and var[3]!='out':
                    ...
                    comp_equation[comp].append(['        '+var[0]+'_out = '+var[0]+'_in;\n'])
            ...
            #declare outputs
                ...
                if var[2] == 'No init':
                    ...
                    f.write('    Modelica.Blocks.Interfaces.RealOutput '+var[0]+'(start='+var[2]+');\n')     
        ...
        for eq in comp_equation[comp]:
            ...
            #write equation with 'if' condition
                ...
                condition_sel = convert_equation_cellml_to_modelica(eq[1].replace('case','').replace(':','').replace('\n',''),var_in_out,comp_equation[comp])
                temp =''
                ...
                if '==' in condition_sel:
                    condition_sel = condition_sel.split()[0] +' >= ' + condition_sel.split()[2] + ' and ' +condition_sel.split()[0] +' <= ' + condition_sel.split()[2]
                equation_1 = convert_equation_cellml_to_modelica(eq[2],var_in_out,comp_equation[comp])
                equation_2 = convert_equation_cellml_to_modelica(eq[-1],var_in_out,comp_equation[comp])
...
with open(library_name+'_cell.mo','w') as f:
    ...
    for comp in comp_var:
        ...
        f.write('    ' + modelica_comp_name+'.Components.'+replace_lower_case_by_capital(comp)+' '+replace_lower_case_by_capital(comp)+';\n')
    for i in real_input:
    ...
    for mapp in map_cellml:
        for var in map_cellml[mapp]:
            if  mapp[1] == '':
                f.write('   connect('+replace_lower_case_by_capital(mapp[0])+'.'+var[0]+', '+var[1]+');\n')
            elif var[0] not in comp_var_in_out[mapp[0]] and var[1] not in comp_var_in_out[mapp[1]]:
                f.write('   connect('+replace_lower_case_by_capital(mapp[0])+'.'+var[0]+', '+replace_lower_case_by_capital(mapp[1])+'.'+var[1]+');\n')
            elif var[0] in comp_var_in_out[mapp[0]]:
                f.write('   connect('+replace_lower_case_by_capital(mapp[0])+'.'+var[0]+'_out, '+replace_lower_case_by_capital(mapp[1])+'.'+var[1]+');\n')
            elif var[1] in comp_var_in_out[mapp[1]]:
                f.write('   connect('+replace_lower_case_by_capital(mapp[0])+'.'+var[0]+', '+replace_lower_case_by_capital(mapp[1])+'.'+var[1]+'_out);\n')
