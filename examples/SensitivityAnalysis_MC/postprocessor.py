...
# Read DoE points
param_file_path = r'D:\xuhu\pyansys-heart\examples\SensitivityAnalysis_MC\scenarios\percent_endo.csv'
...
columns = ['V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'I', 'II', 'III', 'aVR', 'aVL', 'aVF', 'endo_percent']
...
for index, row_data in parameters_df.iterrows():
    ...
    my_simulation_directory = os.path.join(
        workdir, 
        "simulation-EP\\MC_mid",
        str(row_data["percent_endo"]),
    )
    # ep results folder
    ...
    df = df.append(
        {
            ...
            'aVL': (VLA - (VRA + VLL) / 2),
            'aVF': (VLL - (VRA + VLA) / 2),
            'endo_percent': row_data["percent_endo"]
        }, 
        ignore_index=True
    )
