...
@pytest.mark.parametrize(
    "stl_file", ["caps_Left_ventricle.stl", "closed_volume_left_ventricle.stl"]
)
def test_stl_consistency(stl_file):
    ...
    stl_path_ref = os.path.join(get_assets_folder(), "reference_models", "LeftVentricle", stl_file)

    stl_string = read_file(stl_path)
    stl_string_ref = read_file(stl_path_ref)

    assert stl_string == stl_string_ref, "stls are exactly equal"
