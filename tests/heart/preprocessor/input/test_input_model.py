...
def test_simpleware_data():
    path_to_polydata = "D:\development\simpleware-data\simpleware_surface_mesh.vtp"

    part_definitions = {
        "Left ventricle myocardium": {
            "id": 1,
            "enclosed_by_boundaries": {
                "left-ventricle-endocardium": 7009,
                "left-ventricle-epicardium": [7259, 7259, 3007],
                "interface@left-ventricle-myocardium_mitral-valve": 5007,
                "interface@left-ventricle-myocardium_aortic-valve": 4007,
                "septum": 7008,
                # "interface@left-ventricle-myocardium_right-ventricle-myocardium": 6007,
            },
        },
        "Right ventricle myocardium": {
            "id": 2,
            "enclosed_by_boundaries": {
                "right-ventricle-endocardium": 6008,
                "right-ventricle-epicardium": [6259, 4006],
                "interface@left-ventricle-myocardium_pulmonary-valve": 2006,
                "interface@left-ventricle-myocardium_tricuspid-valve": 3006,
                "interface@left-ventricle-myocardium_right-ventricle-myocardium": 6007,
            },
        },
    }
    polydata = pv.PolyData(path_to_polydata)
    model = _InputModel(polydata, scalar="nastran:ref", part_definitions=part_definitions)

    from ansys.heart.preprocessor.mesh.mesher import mesh_from_manifold_input_model

    workdir = os.path.join(conftest.get_workdir(), "simpleware")

    mesh_from_manifold_input_model(model, workdir, "simpleware.msh.h5")
