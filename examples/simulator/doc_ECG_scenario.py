...
if __name__ == "__main__":
    ...
    model.info.mesh_size=float(sys.argv[1])
    ...
    if write_lsdyna_files:
        for writer in (
            ...
            writer.export_databases(
                os.path.join(writer.model.info.workdir, "ECG", "ECG"+sys.argv[1])
                # os.path.join(writer.model.info.workdir, "ECG", "ECG0.25")
            )
    ...
    run_lsdyna(
        ...
        simulation_directory = os.path.join(workdir, "ECG", "ECG"+sys.argv[1]),
    )
